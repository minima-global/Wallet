...
const RefreshButton = ({ onClick }: { onClick: () => void }) => {
    ...
    return (
        <button disabled={disabled} onClick={handleClick} type="button" className={`text-white h-[44px] w-[44px] p-0 border border-grey40 dark:border-mediumDarkContrast rounded-full flex justify-center items-center bg-grey10 dark:bg-darkContrast hover:bg-contrast2 enabled:active:scale-90 disabled:opacity-80 disabled:cursor-not-allowed ${refreshing ? "rotate-[359deg] transition-all" : ""}`}>
