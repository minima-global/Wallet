const GridButton = ({ gridMode, onClick }: { gridMode: 'list' | 'grid', onClick: () => void }) => {
    return (
        <div>
            <button type="button" className="text-white h-[44px] w-[44px] p-0 border border-grey40 dark:border-mediumDarkContrast rounded-full flex justify-center items-center bg-grey10 dark:bg-darkContrast active:scale-90 transition-all " onClick={onClick}>
                {gridMode === 'list' && (
