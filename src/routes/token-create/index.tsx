...
function Index() {
  ...
  const isDisabled = () => {
    if (type === "UPLOAD_IMAGE") {
      return !image
        || !(tokenName.length > 0 && !/\s/.test(tokenName))
        || !(/^\d+$/.test(totalSupply))
          ...
          <div>
            ...
            <form className="flex flex-col gap-6">
              ...
              <Input
                ...
                validationMessage={type === 'SIMPLE' ? "A token name is required and should not contain spaces." : "A token name is required."}
              />
              <Input
                label="Total supply"
              ...
              <div className={`flex flex-col gap-6 ${['SIMPLE'].includes(type) ? 'hidden' : ''}`}>
                <div className="my-2 flex items-center gap-4 text-center text-grey60">
                  ...
                  <div className="relative group">
                    ...
                    <div className="group-hover:opacity-100 pointer-events-none text-white opacity-0 bg-contrast1.5 rounded-md px-3 py-2 absolute text-sm w-full min-w-[210px] top-[calc(100%+14px)] right-[-4px] text-sm text-grey60 before:content-[''] before:absolute before:top-[-4px] before:right-[10px] before:w-[8px] before:h-[8px] before:rotate-45 before:bg-contrast1.5">
